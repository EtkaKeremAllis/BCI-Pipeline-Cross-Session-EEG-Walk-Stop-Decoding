...
CHANNEL_SETS = {
    ...
    'motor13': ['FC3', 'FC1', 'FCz', 'FC2', 'FC4', 'C3', 'C1', 'Cz', 'C2', 'C4',
                'CP3', 'CPz', 'CP4'],
    # Added for the EOG artifact ablation study (see EOG_Artifact_Ablation.md).
    # eog_only: EOG channels alone, to measure how much WALK/STOP-discriminative
    # signal exists in eye-movement channels independent of EEG.
    'eog_only': ['EOG_HL', 'EOG_HR', 'EOG_VA', 'EOG_VB'],
    # motor3_eog: motor3 (C3, Cz, C4) plus the same four EOG channels, to
    # measure how much EEG+EOG gains over EEG-only.
...
def resolve_channels(signals, channel_set):
    ...
    channel_set: 'motor3' | 'motor5' | 'motor9' | 'motor13' | 'eog_only' |
                 'motor3_eog' | 'all_eeg'
...
class DeployableBCIModel:
    ...
    def _apply_channel_normalization(self, raw_signals, reference_seconds=None):
        ...
        for ch in self.channels:
            ...
            if reference_seconds is not None:
                reference_samples = min(len(x), int(round(reference_seconds * self.sampling_rate)))
                if reference_samples <= 0:
                    raise ValueError("Normalization reference interval contains no samples.")
                reference = x[:reference_samples]
            mean = float(np.mean(reference))
            std = max(float(np.std(reference)), EPS)
            normalized[ch] = (x - mean) / std
        return normalized

    def preprocess_continuous(self, raw_signals, normalization_reference_seconds=None):
        """Preprocess the full continuous recording ONCE, not per window."""
        normalized_signals = self._apply_channel_normalization(
            raw_signals, reference_seconds=normalization_reference_seconds
        )
...
def _predict_full_record(edf_path, model, normalization_reference_seconds=None):
    ...
    preprocessed = model.preprocess_continuous(
        signals, normalization_reference_seconds=normalization_reference_seconds
    )
    ...
    if normalization_reference_seconds is not None:
        meta['normalization_reference_seconds'] = float(normalization_reference_seconds)
    return rows, meta
...
def run_validate_timeline(edf_path, events_path, model_dir, output_dir,
    ...
    normalization_reference_seconds = 60.0 if model.channel_normalization == 'zscore' else None
    if normalization_reference_seconds is not None:
        print("[*] Validation z-score statistics use only the first 60 seconds of the EDF.")
    rows_pred, meta = _predict_full_record(
        edf_path, model,
        normalization_reference_seconds=normalization_reference_seconds,
    )
    events_all = parse_events(events_path)
